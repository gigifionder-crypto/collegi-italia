...
una sezione UE-27. Queste due sezioni sono **placeholder superati**: le
versioni autorevoli del Tomo II Nazionale sono quelle in
`../tomo-2-nazionale/` (si veda il README di quella cartella — sono due
prodotti distinti, entrambi mantenuti). Le sezioni equivalenti dentro
questo file non vengono cancellate (vincolo §2.2 del prompt operativo) ma
vanno considerate non autorevoli in caso di conflitto.

## Versioni precedenti

`versioni-precedenti/` contiene le versioni più vecchie o le fonti
intermedie dello stesso documento, conservate per tracciabilità e non
usate come base attiva:

- `opera-integrale-puglia_pre-integrazione-parte-XIV-XV.docx` — il file
  canonico così com'era prima dell'integrazione del 2026-07-06 (senza
  Parte XIV/XV elaborate, solo titoli d'indice).
- `opera-integrale-puglia_RIORDINATO-fonte-parte-XIV-XV.docx` — il file
  da cui è stato estratto il testo di Parte XIV/XV effettivamente
  integrato nel canonico.
- `opera-integrale-puglia_con-68-infografiche-non-integrate.docx` — una
  versione precedente e meno sviluppata (~250.000 parole, 216 tabelle)
  ma con 68 infografiche incorporate. Le infografiche non sono state
  riportate nel file canonico attuale (più sviluppato testualmente ma privo
  di immagini): da valutare in una fase successiva se riprodurle sul
  contenuto corrente.
- `opera-integrale-puglia_v-e1918ebe.docx` — quasi identico al file
